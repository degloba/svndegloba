...
public abstract class JPADataModel<T> extends ExtendedDataModel<T> implements Arrangeable {
    ...
    private String cercaTabular()
    {
		...
		Set<Long> idCaracts = inmobleForm.getFilterValues().keySet(); //ids caracteristiques
		 
		String sql ="";
		Boolean primeraSelect = true;
		 
		// per cada una de les caracteristiques construim la select anidada
		Iterator<Long> idCaractsIt = idCaracts.iterator();
		while (idCaractsIt.hasNext())
		    {
				Long idCaract = (Long) idCaractsIt.next();
    ...
    public T getRowData() {
    	...
    	while(it.hasNext())
    	{
    		...
    		if (inmoble.getIdInmoble() == new Integer((Integer) rowKey))
    			return (T) inmoble;
    	}
