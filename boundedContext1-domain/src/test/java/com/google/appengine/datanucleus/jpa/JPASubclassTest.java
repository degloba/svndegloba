...
import com.degloba.persistence.datanucleus.jpa.IsEmbeddedWithEmbeddedSuperclass;
import com.degloba.persistence.datanucleus.jpa.IsEmbeddedWithEmbeddedSuperclass2;
import com.degloba.persistence.datanucleus.jpa.SubclassesJPA;
import com.degloba.persistence.datanucleus.jpa.SubclassesJPA.Child;
import com.degloba.persistence.datanucleus.jpa.SubclassesJPA.ChildEmbeddedInTablePerClass;
import com.degloba.persistence.datanucleus.jpa.SubclassesJPA.DurableChild;
import com.degloba.persistence.datanucleus.jpa.SubclassesJPA.Grandchild;
import com.degloba.persistence.datanucleus.jpa.SubclassesJPA.Joined;
import com.degloba.persistence.datanucleus.jpa.SubclassesJPA.JoinedChild;
import com.degloba.persistence.datanucleus.jpa.SubclassesJPA.MappedSuperclassChild;
import com.degloba.persistence.datanucleus.jpa.SubclassesJPA.MappedSuperclassParent;
import com.degloba.persistence.datanucleus.jpa.SubclassesJPA.Parent;
import com.degloba.persistence.datanucleus.jpa.SubclassesJPA.SingleTable;
import com.degloba.persistence.datanucleus.jpa.SubclassesJPA.SingleTableChild;
import com.degloba.persistence.datanucleus.jpa.SubclassesJPA.TablePerClass;
import com.degloba.persistence.datanucleus.jpa.SubclassesJPA.TablePerClassChild;
import com.degloba.persistence.datanucleus.jpa.SubclassesJPA.TablePerClassGrandchild;
import com.degloba.persistence.datanucleus.jpa.SubclassesJPA.TablePerClassParentWithEmbedded;
...
public class JPASubclassTest extends JPATestCase {
  ...
  private void testQueryParent(Class<? extends Parent> parentClass, String discriminator) {
    ...
    q = em.createQuery("select aString from " + parentClass.getName() + " b where aString = :p");
    q.setParameter("p", "z8");
  ...
  private void testQueryChild(Class<? extends Child> childClass, String discriminator) {
    ...
    kids = ((List<Child>) em.createQuery(
        "select from " + childClass.getName() + " b where aString = 'a2' order by aString desc").getResultList());
    assertEquals(3, kids.size());
    ...
    q = em.createQuery("select bString, aString from " + childClass.getName() + " b where bString = :p");
    q.setParameter("p", "b2");
    Object[] result = (Object[]) q.getSingleResult();
  ...
  private void testQueryGrandchild(Class<? extends Grandchild> grandchildClass) {
    ...
    grandkids = ((List<Grandchild>) em.createQuery(
        "select from " + grandchildClass.getName() + " b where aString = 'a2' order by aString desc").getResultList());
    assertEquals(3, grandkids.size());
    ...
    grandkids = ((List<Grandchild>) em.createQuery(
        "select from " + grandchildClass.getName() + " b where aString = 'a2' order by cString desc").getResultList());
    assertEquals(3, grandkids.size());
    assertEquals(e2.getKey().getId(), grandkids.get(0).getId().longValue());
    assertEquals(e3.getKey().getId(), grandkids.get(1).getId().longValue());
    assertEquals(e1.getKey().getId(), grandkids.get(2).getId().longValue());

    q = em.createQuery("select bString, aString, cString from " + grandchildClass.getName() + " b where cString = :p");
    q.setParameter("p", "c2");
    Object[] result = (Object[]) q.getSingleResult();
