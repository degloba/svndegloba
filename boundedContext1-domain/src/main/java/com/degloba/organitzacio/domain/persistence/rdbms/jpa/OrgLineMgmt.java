...
public class OrgLineMgmt extends Accountability<Organitzacio , Organitzacio > {
	...
	public OrgLineMgmt(Organitzacio parent, Organitzacio child, Date date) {
		super(parent, child, date);
	}

	public static Organitzacio getParentOfOrganization(
			Organitzacio organitzacio, Date date) {
		return getRepository().createNamedQuery("OrgLineMgmt.getParentOfOrganization")
				.addParameter("organization", organitzacio).addParameter("date", date).singleResult();
	}

	public static List<Organitzacio> findChildrenOfOrganization(
	...
	public static OrgLineMgmt getByResponsible(Organitzacio  responsible, Date date) {
