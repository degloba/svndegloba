...
public class CompresServiceMockWebServerTest {
	...
	/*
	 * @Test void getCompraById() throws Exception { ObjectMapper objectMapper = new
	 * ObjectMapper();
	 * 
	 * //AggregateId compraId = new AggregateId("100"); //AggregateId comandaId =
	 * new AggregateId("200"); //AggregateId comandaKOId = new AggregateId("300");
	 * 
	 * String compraId = "100"; String comandaId = "200"; String comandaKOId =
	 * "300";
	 * 
	 * CompraDto mockCompra = new CompraDto(compraId);
	 * 
	 * // Utilitzem el métode en cola de MockWebServer per posar en cola una
	 * resposta de prova en el servidor web // Cuando la llamada API real se realiza
	 * desde el método getCompraById(AggregateId compraId) en la classe
	 * CompraService, // MockWebServer respondra amb el codi auxiliar en cola.
	 * mockBackEnd.enqueue(new MockResponse()
	 * .setBody(objectMapper.writeValueAsString(mockCompra))
	 * .addHeader("Content-Type", "application/json"));
	 * 
	 * ///////// Mono<CompraDto> enviamentMono =
	 * compresService.getCompraById(compraId);
	 * 
	 * StepVerifier.create(enviamentMono) .expectNextMatches(enviament ->
	 * enviament.getComandaId() .equals(comandaId)) .verifyComplete();
	 * 
	 * 
	 * // Tambié es possible que vulguem assegurar-nos de que MockWebServer ha rebut
	 * la HttpRequest correcta. // MockWebServer té un métode (takeRequest) que
	 * retorna una instancia de RecordedRequest. RecordedRequest recordedRequest =
	 * mockBackEnd.takeRequest();
	 * 
	 * assertEquals("GET", recordedRequest.getMethod()); assertEquals("/compra/100",
	...
	public void testRecursivelyCreateObjectNestedBean() throws Exception {
		 ...
		 * WebClient.create().method(HttpMethod.POST).uri("/resource")
		...
		BodyInserter<MultiValueMap<String, Object>, ClientHttpRequest> inserter2 = BodyInserters
				.fromMultipartData(formValues);
