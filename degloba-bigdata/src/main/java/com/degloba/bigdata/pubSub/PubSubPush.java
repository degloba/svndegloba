...
public class PubSubPush  {
	 ...
	 public int verificaMessage(String token, String message) {
		 
	    String pubsubVerificationToken = System.getenv("PUBSUB_VERIFICATION_TOKEN");
	    
	    // Do not process message if request token does not match pubsubVerificationToken
	    if (token.compareTo(pubsubVerificationToken) != 0) {
	      // resp.setStatus(HttpServletResponse.SC_BAD_REQUEST);
	      return 1;
	    }
	    // parse message object from "message" field in the request body json
	    // decode message data from base64
	    Message messageObject = getMessage(message);
	    try {
  ...
  private Message getMessage(String requestBody)  {
