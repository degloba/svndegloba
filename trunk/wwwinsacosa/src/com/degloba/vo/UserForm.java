...
import com.degloba.util.MessageFactory;
...
public class UserForm  {
	...
	public String editProfile() throws Exception{
		...
		Usuaris usuari = r.editPerfil(this.getNomUsuari());
				
		this.setNom(usuari.getNom());
		this.setCognoms(usuari.getCognoms());
		this.setNomUsuari(usuari.getNomusuari());
	...
	public String  saveUser() throws Exception{
		if (validateData()){
			...
			if(!loginat){
				...
				usuariHib.setNomusuari(this.getNomUsuari());
				...
				usuariHib.setNomusuari(this.nomUsuari);
				usuariHib.setAdreca(this.getAdreca());
				usuariHib.setEmail(this.getEmail());
				usuariHib.setCognoms(this.getCognoms());
				usuariHib.setNom(this.getNom());				
	...
	public boolean validUser(){
		String username = nomUsuari.trim();
		boolean validUser=true;
		
		//Interfaces interfaceS = serviceFinder.findBean("Interfaces");
		Usuari_Impl r = new Usuari_Impl();
		Usuaris usuari = new Usuaris();
		usuari.setNomusuari(username);
		r.usuariValid(usuari);
			
		return r.usuariValid(usuari);
	}
	...
	private boolean validateData() throws Exception {
		...
		if(getNomUsuari() == ""){
			...
			if((username.length())==0){
				ctx.addMessage("UserForm:nomUsuari", new FacesMessage(FacesMessage.SEVERITY_ERROR, mf.getMessage("errorFirstNameUsuariBlank"), null));
				status = false;		
			}

				
			if((!(validUser()))){
				ctx.addMessage("UserForm:nomUsuari",new FacesMessage(FacesMessage.SEVERITY_ERROR, mf.getMessage("errorValidNomUsuari"), null));
				status = false;
			}
		

			if((username.length())<6 || (username.length())>20 ){
				ctx.addMessage("UserForm:nomUsuari", new FacesMessage(FacesMessage.SEVERITY_ERROR, mf.getMessage("errorFirstNameUsuariLength"), null));
				status = false;
			}

			String iChars = "!@#$%^&*()+=-[]\\\';,/{}|\":<>?";
			for (int i = 0; i < nomUsuari.length(); i++) {
				if (iChars.indexOf(nomUsuari.charAt(i)) != -1) {
					ctx.addMessage("UserForm:nomUsuari", new FacesMessage(FacesMessage.SEVERITY_ERROR, mf.getMessage("errorFirstNameUsuari"), null));
					status = false;
					break;
				}
			}
