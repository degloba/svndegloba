...
jsf.util={};
jsf.util.chain=function(F,D){if(arguments.length<3){return true
}var A=(typeof F==="object")?F:null;
for(var B=2;
B<arguments.length;
...
E.adp=D;
var B=0;
for(var A in C){if(C.hasOwnProperty(A)){var F=document.createElement("input");
F.type="hidden";
F.name=A;
F.value=C[A];
E.appendChild(F);
...
mojarra.ab=function ab(C,D,F,A,B,E){if(!E){E={}
}if(F){E["javax.faces.behavior.event"]=F
}if(A){E.execute=A
}if(B){E.render=B
}jsf.ajax.request(C,D,E)
};;/*
...
var getFormElement=function(sourceElement){if($(sourceElement).is("form")){return source
...
}})(jQuery);;(function(E,C){C.Event=C.Event||{};
...
}(window.RichFaces,jQuery));;(function(E,C){C.ui.Message=function(I,H){G.constructor.call(this,I,H,A);
...
})(jQuery,window.RichFaces);;(function(D,A){A.ui=A.ui||{};
...
A.ui.TabPanel=A.ui.TogglePanel.extendClass({name:"TabPanel",init:function(D,C){A.ui.TogglePanel.call(this,D,C);
this.items=[];
this.isKeepHeight=C.isKeepHeight||false
}})
...
this.timeoutId=window.setTimeout(function(){if(this.isVisible){T.__hide(S)
}},200)
...
})(jQuery,RichFaces);;(function(D,U){U.ui=U.ui||{};
